...
def build_sampling_kwargs(temperature, top_p):
    """将 OpenAI 兼容采样参数转换为 Transformers 生成参数。"""
    if temperature <= 0:
        ...
        def _generate():
            try:
                model.generate(
                    ...
                    streamer=streamer,
                    **build_sampling_kwargs(temperature, top_p),
                )
        ...
        if request.stream:
            ...
            with torch.no_grad():
                generated_ids = model.generate(
                    ...
                    pad_token_id=tokenizer.pad_token_id,
                    eos_token_id=tokenizer.eos_token_id,
                    **build_sampling_kwargs(request.temperature, request.top_p),
                )
